...
function useSkill(skillId){
  ...
  if(sk.slotCost&&!G._timeStopActive&&!G._arcaneTranscendence){
    ...
    if(G._graceSpellflame&&Math.random()<0.15){
...
function doSkillEffect(effect, sk){
  switch(effect){
    ...
    case 'fire_bolt':{AUDIO.sfx.burn();
      ...
      if(G.classId==='wizard'&&G.talents.includes('Spell Power'))fbd=Math.ceil(fbd*1.15);
    ...
    case 'magic_missile':{
      ...
      // Spell Power: +15% spell damage
      if(G.classId==='wizard'&&G.talents.includes('Spell Power'))t=Math.ceil(t*1.15);
      // Arcane Transcendence: +30% spell damage
    ...
    case 'fireball':{
      ...
      addConditionAllEnemies('Burning',3);
    ...
    case 'divine_smite':{
      ...
      let d=roll(8)+roll(8)+roll(8)+md(G.stats.cha);
      // Avatar of Light capstone: spend ALL Holy Power for +1d8 per point
      if(G.capstoneUnlocked&&G.classId==='paladin'&&G.res>0){
        const spent=G.res;for(let i=0;i<spent;i++)d+=roll(8);
        G.res=0;log('👼 Avatar of Light: '+spent+' Holy Power channeled! (+'+spent+'d8)','s');
      }
    ...
    case 'volley':{
      ...
      if(vTargets.length<=1){
        // Single target — all arrows at it (original behavior)
        for(let i=0;i<3;i++){
    ...
    case 'sacred_flame':{
      ...
      const d=roll(10)+scalingBonus+(G.magBonus||0)+(G.subclassUnlocked&&G.classId==='cleric'?roll(4):0)+(G.talents.includes('Radiant Soul')?roll(4):0)+md(G.stats.wis);
      ...
      if(G.classId==='cleric'&&G._wrathOfGod&&!G._wrathOfGodUsed&&G.currentEnemy&&G.currentEnemy.hp>0){
        ...
        const d2=roll(10)+scalingBonus+(G.subclassUnlocked?roll(4):0)+(G.talents.includes('Radiant Soul')?roll(4):0)+md(G.stats.wis);
    ...
    case 'empowered_blast':{
      ...
      const ebDmg0=32+Math.max(0,md(G.stats.int))+(G.magBonus||0);
      // Spell Power: +15% spell damage
      let ebDmg1=G.talents.includes('Spell Power')?Math.ceil(ebDmg0*1.15):ebDmg0;
      // Arcane Transcendence: +30% spell damage
...
function dealToEnemy(dmg,crit,source){
  ...
  if(G._graceGodslayer&&G.currentEnemy.isBoss) dmg=Math.ceil(dmg*1.2);
  // Legendary Grace — The First Blade: first attack each fight deals double damage
  if(G._graceFirstBlade&&!G._graceFirstBladeUsed){G._graceFirstBladeUsed=true;dmg=dmg*2;log('🗡️ The First Blade: DOUBLE DAMAGE!','s');}
...
function onEnemyDied(){
  ...
  if(e.isBoss){
    ...
    const doGraceDrop = (onDone) => {
      if(typeof generateGrace==='function' && typeof showGraceDrop==='function'){
  ...
  if(!G._fightDamageTaken && _fightGold>0){
    const flawlessBonus = Math.floor(_fightGold * 0.50);
    if(flawlessBonus>0){
  ...
  if(!G.campUnlocked&&G.dungeonFights>=G.dungeonGoal){
    ...
    if(G.hp>G.maxHp*0.5){
      const survivalBonus=Math.floor(15+(G.zoneIdx||0)*1.5);
...
function onPlayerDied(){
  ...
  if(G.lives<=0){
    // Run over — offer salvage first, then wipe state
    log('💀 No lives remaining. Your journey ends here.','e');
    setTimeout(()=>{
      showDeathOverlay(()=>{
        // Show salvage prompt — player picks 1 item before G is nulled
        showSalvagePrompt(()=>{
          ...
          if(typeof showScreen==='function') showScreen('saveSlots');
        });
      });
    },800);
    return;
