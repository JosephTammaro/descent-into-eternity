...
function doEnemyTurn(){
  ...
  if(G.wildShapeHp<=0&&G.skillCooldowns['wild_shape']){
    ...
    if(G.wildShapeActive){
      ...
      const pSpr=document.getElementById('playerSprite');
      if(pSpr&&CLASS_SPRITES[G.classId])renderSprite(CLASS_SPRITES[G.classId],13,pSpr);
...
function doEnemyAttack(e){
  ...
  if(G.wildShapeHp>0){
    ...
    if(G.wildShapeHp<=0){
      ...
      const pSpr=document.getElementById('playerSprite');
      if(pSpr&&CLASS_SPRITES.druid)renderSprite(CLASS_SPRITES.druid,13,pSpr);
      if(pSpr)pSpr.style.filter='';
      const exitBtn=document.getElementById('exitWildShapeBtn');
      if(exitBtn)exitBtn.style.display='none';
      swapBearBars(false);
    ...
    const firstClearFlow = () => {
      const afterLevelUps = () => {
        doGraceDrop(() => {
          ...
          log('✨ '+nextZoneName+' awaits. Return to Elderfen.','l');
          // Show lore beat then send to town — G stays alive so player keeps gold/inventory
...
function onPlayerDied(){
  ...
  // Lives remain — send to campfire if reached this run, else back to Elderfen
  showDeathOverlay(()=>{
    if(!G)return;
    G._dyingFlag=false;
    ...
    if(G._campReached){
      ...
      log('💀 You died — returning to Elderfen...','e');
      if(typeof fadeToScreen==='function'){
        fadeToScreen('town',()=>{ if(typeof showTownHub==='function') showTownHub(); });
      } else if(typeof showTownHub==='function'){
        showTownHub();
