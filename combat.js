  ...
  switch(effect){
    ...
    case 'volley':{
      ...
      for(let i=0;i<3;i++){
        ...
        dealToEnemy(d,r.crit,'Volley 🏹 arrow '+(i+1));
        t+=d;arrowsHit++;
        if(G.currentEnemy&&G.currentEnemy.hp<=0)break;
      }
...
function onEnemyDied(){
  ...
  if(e.isBoss){
    ...
    G.currentEnemy=null;
    G._bossSequenceActive = true; // block doEnemyTurn during this window
    ...
    // ── First clear: boss victory → grace → fade → slot detail ──
    const firstClearFlow = () => {
      doGraceDrop(() => {
        ...
        const nextZoneName = (typeof ZONES!=='undefined'&&ZONES[loreIdx+1]) ? ZONES[loreIdx+1].name : 'the next zone';
        log('🗺 '+nextZoneName+' unlocked for your next run!','l');
        G = null;
        // Fade to black then open the slot detail screen
        setTimeout(()=>{
          if(typeof fadeToScreen==='function'){
            fadeToScreen('slotDetail', ()=>{
              if(typeof openSlotDetail==='function'&&slot) openSlotDetail(slot);
            });
          } else {
    ...
    // ── Repeat clear: level-up → grace → lore → map ──────────
...
function onPlayerDied(){
  ...
  // Lives remain — show YOU DIED then campfire with lives warning
  G.restsThisZone=0;
  G.campUnlocked=true;
  G.dungeonFights=0;
  showDeathOverlay(()=>{
    ...
    showCampfire();
    // Flash lives warning after campfire loads
    setTimeout(()=>{
      if(typeof renderLives==='function') renderLives();
      const cfLives=document.getElementById('cfLives');
      if(cfLives){
        cfLives.style.animation='';
        cfLives.style.color=G.lives===1?'var(--red2)':'var(--gold)';
        void cfLives.offsetWidth;
        cfLives.style.transition='';
      }
    },200);
  });
}
