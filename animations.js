...
const _FRAME_TIMING = {
  fighter:   [220, 160, 220],  // medium confident pace
  barbarian: [110, 210, 110],  // fast drop, hold at raised pose
  wizard:    [320, 280, 320],  // slow ethereal pulse
  rogue:     [140,  90, 140],  // quickest flicker
  paladin:   [480, 180, 480],  // holds neutral very long, brief movement
  ranger:    [210, 140, 210],  // alert but controlled
  cleric:    [290, 210, 290],  // gentle prayer cycle
  druid:     [400, 320, 400],  // slowest and most organic
};
...
const Anim = {
  ...
  startPlayerIdle(classId){
    const cid = classId || (window.G && window.G.classId);
    if(!cid) return;
    if(cid === 'druid' && window.G && window.G.wildShapeHp > 0) return;
    _addIdleClass(cid);
    _startSwapper(cid);
  },
  ...
  playerAttack(onPeak){
    ...
    _applyClass('playerSprite', 'anim-attack', DURATION, ()=>{
      const cid = _frameClassId || (window.G && window.G.classId);
