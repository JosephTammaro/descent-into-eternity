...
function cfDoRest(){
  ...
  G.hp=Math.min(G.maxHp, Math.max(G.hp, Math.floor(G.maxHp*0.75))); // Restore to 75% HP (or keep current if higher)
  ...
  if(el){el.textContent='✓ Rested! HP restored to 75%. Resources and conditions restored.';el.style.color='var(--green2)';el.style.display='block';}
...
function travelToZone(zoneIdx){
  ...
  G.dungeonFights=0;G.campUnlocked=false;G.dungeonGoal=zoneIdx<=1?4:5;
  G.restsThisZone=0;
