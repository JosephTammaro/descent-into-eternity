...
function closeLevelUp(){
  ...
  if(data&&data.subclassUnlock){
    ...
    if(data.talentUnlock) G._pendingTalentAfterSubclass=true;
    showSubclassScreen();
    return;
  }
...
function confirmUltimate(type){
  ...
  if(isCapstone){
    ...
    // Track rest usage for once-per-rest ultimates
...
function renderEquipSlots(){
  if(!G)return;
  const slots=[{k:'weapon',l:'Weapon'},{k:'armor',l:'Armor'},{k:'ring',l:'Ring'},{k:'offhand',l:'Off-Hand'}];
...
function renderUpgrades(){
  if(!G)return;
  document.getElementById('upgradeRows').innerHTML=G.upgrades.filter(u=>!u.bought).slice(0,4).map(u=>`
    <div class="upgrade-row ${G.gold<u.cost?'cant':''}" onclick="buyUpgrade('${u.id}')">
      <span>${u.icon}</span>
      <span style="flex:1;color:var(--dim)">${u.name}<br><span style="font-size:12px">${u.desc}</span></span>
      <span class="upg-cost">🪙${u.cost}</span>
    </div>
  `).join('')||'<div style="padding:8px;font-size:13px;color:var(--dim);">All bought!</div>';
}
...
function startPlayerAnim(){
  stopPlayerAnim();
  if(!G||!G.classId)return;
  if(G.classId==='druid'&&G.wildShapeHp>0)return;
  const el=document.getElementById('playerSprite');
  if(!el)return;
  const cls=CLASS_ANIM_PROFILE[G.classId];
  if(!cls)return;
...
function stopPlayerAnim(){
  const el=document.getElementById('playerSprite');
  if(el)_IDLE_CLASSES.forEach(c=>el.classList.remove(c));
...
function cfRestHTML(){
  ...
  const restLimit = d==='hardcore' ? 2 : d==='hard' ? 3 : null;
  const restsLeft = restLimit !== null ? Math.max(0, restLimit - (G.restsThisZone||0)) : null;
  const exhausted = restLimit !== null && restsLeft <= 0;
  const limitColor = d==='hardcore' ? 'var(--red2)' : '#e67e22';
  const restPips = restLimit !== null
    ? Array.from({length:restLimit},(_,i)=>i<restsLeft?'🔥':'💀').join('  ')
    : '';
  const warnColor = restsLeft===0?'#cc0000':restsLeft===1?'#e67e22':limitColor;
  const restCounterHTML = restLimit !== null
    ? `<div class="rest-limit-bar" style="border-color:${warnColor};color:${warnColor};">
        ${restsLeft===0?'☠ NO RESTS REMAINING — YOU CANNOT HEAL':'⚠ RESTS LEFT THIS ZONE: '+restsLeft+' / '+restLimit}
        <span class="rest-pips">${restPips}</span>
      </div>`
    : '';
  return `<div class="rest-panel">
    ...
    ${exhausted
      ? `<div style="font-family:'Press Start 2P',monospace;font-size:8px;color:var(--red2);margin-bottom:12px;line-height:1.8;">☠ NO RESTS REMAIN<br><span style="color:var(--dim);">Push on or use potions.</span></div>`
      : G._longRestUsed
    ...
    ${d==='hard' ? `<button class="btn" style="font-size:9px;padding:10px 24px;margin-top:8px;border-color:#3a5a3a;" onclick="cfDoShortRest()">💤 SHORT REST (50% HP, free)</button>` : ''}
...
function cfDoRest(){
  ...
  // Check rest limit (Hard: 3, Hardcore: 2)
  const restLimit = G.difficulty==='hardcore' ? 2 : G.difficulty==='hard' ? 3 : null;
  ...
  if(G.classId==='druid'){const p=document.getElementById('playerSprite');if(p&&CLASS_SPRITES.druid)renderSprite(CLASS_SPRITES.druid,10,p);if(p)p.style.filter='';startPlayerAnim();}
  ...
  if(G.difficulty==='hard'||G.difficulty==='hardcore') G.restsThisZone=(G.restsThisZone||0)+1;
  const el=document.getElementById('restResult');
...
function cfShopHTML(cat){
  ...
  const CLASS_ALLOWED={
    fighter:   {weapon:['sword','silverSword','greatsword','handaxe'],armor:['leather','paddedArmor','chainmail','scaleMail','plateArmor'],offhand:['shield','ironShield'],ring:true,consumable:true},
    wizard:    {weapon:['staff','wand','arcaneOrb'],armor:['paddedArmor','robes','shadowLeather'],offhand:['spellbook','arcaneOrb2'],ring:true,consumable:true},
    rogue:     {weapon:['dagger','voidDagger','sword','silverSword'],armor:['leather','shadowLeather','paddedArmor'],offhand:['shield'],ring:true,consumable:true},
    paladin:   {weapon:['sword','silverSword','greatsword'],armor:['chainmail','scaleMail','plateArmor','leather'],offhand:['shield','ironShield'],ring:true,consumable:true},
    ranger:    {weapon:['bow','longbow','dagger'],armor:['leather','shadowLeather','scaleMail'],offhand:['shield','spellbook'],ring:true,consumable:true},
    barbarian: {weapon:['club','handaxe','greatsword','sword'],armor:['leather','paddedArmor','chainmail'],offhand:[],ring:true,consumable:true},
    cleric:    {weapon:['staff','wand','club','sword'],armor:['chainmail','scaleMail','robes','leather'],offhand:['shield','ironShield','spellbook'],ring:true,consumable:true},
    druid:     {weapon:['staff','druidStaff','bow','club'],armor:['leather','robes','paddedArmor'],offhand:['spellbook','shield'],ring:true,consumable:true},
  };
  ...
  function itemAllowedForClass(item){
    ...
    if(item.type==='accessory')return true; // rings always allowed
    if(item.forClass)return item.forClass===G.classId; // class set items — already filtered by drop, but show in inventory
    if(item.set&&!item.forClass)return true; // generic sets always available
    ...
    return !!allowedIds; // true means all allowed (rings)
  }
  const allItems=ITEMS.filter(i=>i.type!=='material'&&i.value>0&&itemAllowedForClass(i));
  const cats=[
    {k:'all',l:'All'},
    {k:'weapon',l:'Weapons'},
    {k:'armor',l:'Armor'},
  ...
  // Build equipped bar
  const slots=[{k:'weapon',l:'WEAPON'},{k:'armor',l:'ARMOR'},{k:'ring',l:'RING'},{k:'offhand',l:'OFF-H'}];
...
function updateCampBtn(){
  ...
  if(!btn)return;
  const unlocked=G.campUnlocked||G.bossDefeated[G.zoneIdx];
  if(unlocked){
    btn.disabled=false;
...
function continueJourney(){
  ...
  if(G.bossDefeated[G.zoneIdx]&&G.zoneIdx<ZONES.length-1){
    // Auto-travel to the next zone — no need to make the player hunt the map button
    travelToZone(G.zoneIdx+1);
  } else if(G.bossDefeated[G.zoneIdx]&&G.zoneIdx>=ZONES.length-1){
    ...
    G.dungeonGoal=G.difficulty==='normal'?3:5;
    showScreen('game');
    renderAll();
    updateCampBtn();
    spawnEnemy();
...
function renderWorldMap(){
  ...
  MAP_NODES.forEach(node=>{
    ...
    // Main node unlock/clear state
    const cleared   = isMainNode && G.bossDefeated[node.zone];
    const isCur     = isMainNode && node.zone === G.zoneIdx;
    const unlocked  = isMainNode && (node.zone===0 || G.bossDefeated[node.zone-1] || cleared);
    ...
    if(isBranchNode){
      ...
      branchUnlocked = bDef && G.bossDefeated && G.bossDefeated[bDef.unlocksAfter] && !branchCleared;
    }
    ...
    const labelSuffix = isBranchNode && !branchCleared ? '\n★ SIDE QUEST' : '';
    el.innerHTML=`<div class="mn-dot">${dotChar}</div><div class="mn-label">${node.label}${labelSuffix}</div>`;
...
function travelToZone(zoneIdx){
  ...
  G.dungeonFights=0;G.campUnlocked=false;G.dungeonGoal=G.difficulty==='normal'?3:5;
  G.restsThisZone=0;
  G.hp=G.maxHp;
...
//  CHAR SHEET
// ══════════════════════════════════════════════════════════
function toggleInlineCharSheet(){
  const body=document.getElementById('csiBody');
  const arrow=document.getElementById('csiArrow');
  if(!body)return;
  const open=body.classList.toggle('open');
  if(arrow)arrow.textContent=open?'▲':'▼';
  if(open&&G)renderInlineCharSheet();
}
...
function renderInlineCharSheet(){
  ...
  body.innerHTML=`
    <div class="csi-cols">
      ...
      <div class="csi-col">
        ${section('EQUIPMENT')}
        ${['weapon','armor','ring','offhand'].map(slot=>{const item=G.equipped[slot];return row(slot.toUpperCase(),item?item.icon+' '+item.name:'—',item?rarityColor[item.rarity]:'var(--dim)');}).join('')}
...
function renderCharSheet(){
  ...
  document.getElementById('csGrid').innerHTML=`
    ...
    <div class="cs-panel">
      ...
      ${['weapon','armor','ring','offhand'].map(slot=>{const item=G.equipped[slot];return`<div class="cs-row"><span style="color:var(--dim)">${slot.toUpperCase()}</span><span style="color:${item?rarityColor[item.rarity]:'var(--dim)'}">${item?item.icon+' '+item.name:'—'}</span></div>`;}).join('')}
...
function restorePauseMenu(){
  ...
  pauseBox.innerHTML=`
    ...
    <div class="pause-btns">
      ...
      <button class="btn" onclick="showCharSheet()">📜 CHARACTER SHEET</button>
      <button class="btn" style="border-color:#2a4a2a;color:#3dbd52;" onclick="openBugLog()">🐛 BUG LOG</button>
...
function showScreen(id){
  ...
  if(id==='game'){
    // Restart player anim when coming back to game screen
    if(G&&CLASS_SPRITES[G.classId])startPlayerAnim();
  } else if(id!=='game'){
    // Stop on non-game screens (campfire has its own cleanup)
  }
}
