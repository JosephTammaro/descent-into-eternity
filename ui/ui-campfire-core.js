...
function cfRestHTML(){
  return `<div class="rest-panel">
    ...
    <div class="rest-preview">
      <div class="rest-item">${iconHTML('health')} HP<span>Restore to ${Math.floor(G.maxHp * Math.min(1.0, (G._rareEventFlags&&G._rareEventFlags.lastCampfireWeaken ? 0.50 : 0.75) + (G._campfireHealBonus||0)))} / ${G.maxHp}</span></div>
...
function cfDoRest(){
  ...
  const baseRestPct=G._rareEventFlags&&G._rareEventFlags.lastCampfireWeaken?0.50:0.75;
  ...
  G.hp=Math.min(G.maxHp, Math.max(G.hp, Math.floor(G.maxHp*restPct))); // Restore to 75% HP (or 50% if weakened by Last Campfire event); won't reduce HP if already above threshold
